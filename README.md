# costctl - G15 XBrain W6 side challenge
...
./costctl.py cost --tag Application=BuildForce --days 7
...
- [x] (optional stretch) `clean` → `pytest tests/test_clean.py` green; `idle` + `migrate-gp3` also implemented
...
- [ ] Post link in Slack `#w6-sidechallenge` thread:
      `G15 - https://github.com/Build-Force/buildfoce_be.git - implemented: list, cost, terminate, tag, clean, idle, migrate-gp3 (25/25 tests passing)`
...
## `make test` final score

**25 / 25 passing** (10 helpers + 7 list + 4 terminate + 4 clean)

```
tests/test_clean.py      4 passed
tests/test_common.py    10 passed
tests/test_list.py       7 passed
tests/test_terminate.py  4 passed
```
...
*G15 - XBrain W6 Side Challenge - submitted 2026-05-22*
